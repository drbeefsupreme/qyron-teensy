...
void setup() {
  ...
  num_filesM = enumerateGIFFiles(M_DIRECTORY, false);
  debug("entering loop...");
}
...
void loop() {
  ...
  interface(
    ...
    gifM, "gifM: mil. @a: none @return: none",
...
//    getTemperature, "getTemperature: displays CPU temp. @a: none @return: none."
  ...
  if(gifs_loop >= 0) {
    ...
    if(gif_dir == 13) {
        gif_loop(M_DIRECTORY, num_filesM);
    }
  }
}
